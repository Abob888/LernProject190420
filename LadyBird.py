...
from PyQt5.QtWidgets import (QMainWindow, QFrame, QDesktopWidget, QApplication,
                             ...
                             QLabel, QWidget, QAction, qApp, QMenuBar, QLCDNumber)
...
class LadyBird(QMainWindow):
    ...
    def initUI(self):
        ...
        '''self.bigSize = QAction(QIcon('bigs.png'), '&Big size', self)
        self.bigSize.setShortcut('Ctrl+B')
        self.bigSize.setStatusTip('Big size window')
        self.bigSize.triggered.connect(self.on_max)
        self.sizeMenu = self.menubar.addMenu('&Window')
        self.sizeMenu.addAction(self.bigSize)

        self.normSize = QAction(QIcon('norms.png'), '&Normal size', self)
        self.normSize.setShortcut('Ctrl+N')
        self.normSize.setStatusTip('Normal size window')
        self.normSize.triggered.connect(self.on_norm)
        self.sizeMenu.addAction(self.normSize)'''
    ...
    def on_displ(self):
        global modalWindow
        modalWindow = QtWidgets.QWidget(lbrace, QtCore.Qt.Window)
        modalWindow.setWindowTitle('The Score')
        modalWindow.resize(500, 700)
        modalWindow.setWindowModality(QtCore.Qt.WindowModal)
        modalWindow.setAttribute(QtCore.Qt.WA_DeleteOnClose, True)
        modalWindow.moove(20, 20)
        modalWindow.show()
        '''try:
    ...
    def startLB(self):
        ...
        while c >= 112:
            ...
            for b in range(5):
                ...
                kori.insert(b, cy)
                korilab2 = QLabel(self)
                korilab2.setText(str(kori[b]))
                korilab2.move(b * 100 + 15, 450)
                korilab2.show()
        ...
        '''for f in range(5):
            winner = QLabel(self)
            winner.setText(str(kori[f]))
            winner.move(f * 100 + 15, 5)
            winner.show()'''
